use super::routing::{orthogonal_route, RouteRect, RouteRequest};
use super::*;
...
fn property_rect(value: &IbdPropertyPresentation) -> RouteRect {
    RouteRect { x: value.x, y: value.y, width: value.width, height: value.height }
}
...
fn ibd_end_for_presentation(diagram: &IbdDiagram, presentation_id: &str) -> Result<(ConnectorEnd, RouteRect), String> {
    if let Some(port) = diagram.boundary_ports.iter().find(|port| port.id == presentation_id) {
        return Ok((ConnectorEnd::boundary(parse_element_id(&port.element_id)?), port_rect(port)));
    }
    for property in &diagram.properties {
        if property.id == presentation_id {
            ...
            let end = if path.is_empty() { ConnectorEnd::role(role_id) } else { ConnectorEnd { property_path: path, role_id, port_id: None } };
        ...
        if let Some(port) = property.ports.iter().find(|port| port.id == presentation_id) {
            ...
            return Ok((ConnectorEnd::nested_port(path, parse_element_id(&port.element_id)?), port_rect(port)));
        }
    }
    Err(format!("IBD endpoint presentation not found: {presentation_id}"))
}
...
fn lane_index(diagram: &IbdDiagram, source_id: &str, target_id: &str) -> usize {
    diagram.connectors.iter().filter(|edge| {
        (edge.source_presentation_id == source_id && edge.target_presentation_id == target_id)
            || (edge.source_presentation_id == target_id && edge.target_presentation_id == source_id)
    }).count()
}

pub fn route_ibd_edge(diagram: &IbdDiagram, source_id: &str, target_id: &str) -> Result<Vec<DiagramPoint>, String> {
...
pub fn validate_ibd_diagrams(project: &Project, diagrams: &[IbdDiagram]) -> Result<(), String> {
    ...
    for diagram in diagrams {
        ...
        if !matches!(context.kind, ElementKind::Block | ElementKind::AssociationBlock) {
            return Err(format!("IBD context must be a Block or AssociationBlock: {}", diagram.context_block_id));
        }
        let owner = project.element(parse_element_id(&diagram.owner_id)?).map_err(|error| error.to_string())?;
        ...
        for property in &diagram.properties {
            ...
            let element = project.element(parse_element_id(&property.element_id)?).map_err(|error| error.to_string())?;
            if !matches!(element.kind, ElementKind::PartProperty | ElementKind::ReferenceProperty) {
                return Err(format!("IBD structural node must reference PartProperty or ReferenceProperty: {}", property.element_id));
            }
            let path = parse_path(&property.property_path)?;
            if path.last().copied() != Some(element.id) {
                return Err(format!("IBD property path must terminate at the presented property: {}", property.element_id));
            }
            ...
            for port in &property.ports {
                ...
                let port_element = project.element(parse_element_id(&port.element_id)?).map_err(|error| error.to_string())?;
                if !port_element.is_port() {
                    return Err(format!("IBD port presentation does not reference a port: {}", port.element_id));
                }
                let end = ConnectorEnd::nested_port(parse_path(&port.property_path)?, port_element.id);
                project.validate_connector_end(context_id, &end).map_err(|error| error.to_string())?;
            }
        }
        for port in &diagram.boundary_ports {
            if !presentation_ids.insert(&port.id) {
        ...
        for edge in &diagram.connectors {
            if !presentation_ids.insert(&edge.id) {
                return Err(format!("duplicate IBD connector presentation id: {}", edge.id));
            }
            let relationship = project.relationship(parse_relationship_id(&edge.relationship_id)?).map_err(|error| error.to_string())?;
            if relationship.kind != RelationshipKind::Connector {
                return Err(format!("IBD connector edge does not reference a semantic Connector: {}", edge.relationship_id));
            }
            ...
            let semantic = relationship.connector.as_ref().ok_or("Connector semantics missing")?;
            if semantic.source != source || semantic.target != target {
                return Err(format!("IBD presentation endpoints do not match semantic Connector: {}", edge.relationship_id));
            }
...
pub fn create_ibd(
    ...
    let owner_id_parsed = parse_element_id(&owner_id)?;
    {
        ...
        let owner = project.element(owner_id_parsed).map_err(|error| error.to_string())?;
        if !matches!(owner.kind, ElementKind::Model | ElementKind::Package) {
            return Err("IBD repository owner must be a Model or Package".into());
        }
    }
    if state.ibd_diagrams.lock().map_err(|_| "IBD lock poisoned")?.iter().any(|d| d.context_block_id == context_block_id) {
        return Err("this Block already has an Internal Block Diagram".into());
    }
    let id = DiagramId::new().to_string();
    state.ibd_diagrams.lock().map_err(|_| "IBD lock poisoned")?.push(IbdDiagram {
        id: id.clone(), name, context_block_id, owner_id,
        properties: Vec::new(), boundary_ports: Vec::new(), connectors: Vec::new(),
    });
    Ok(id)
}
...
pub fn populate_ibd_from_context(
    ...
    let diagram = diagrams.iter_mut().find(|d| d.id == diagram_id).ok_or("IBD not found")?;
    ...
    for feature in project.children(context) {
        match feature.kind {
            ElementKind::PartProperty | ElementKind::ReferenceProperty => {
                if diagram.properties.iter().any(|p| p.element_id == feature.id.to_string()) { continue; }
                diagram.properties.push(IbdPropertyPresentation {
                    ...
                    x, y, width: 190.0, height: 100.0, ports: Vec::new(),
                });
                x += 240.0;
                if x > 780.0 { x = 120.0; y += 180.0; }
            }
            ElementKind::ProxyPort | ElementKind::FullPort => {
                if diagram.boundary_ports.iter().any(|p| p.element_id == feature.id.to_string()) { continue; }
                diagram.boundary_ports.push(IbdPortPresentation {
                    id: uuid::Uuid::new_v4().to_string(), element_id: feature.id.to_string(),
                    property_path: Vec::new(), x: 55.0, y: 100.0 + diagram.boundary_ports.len() as f64 * 70.0, size: 16.0,
                });
            }
            _ => {}
        }
...
pub fn add_nested_port_to_ibd(
    ...
    let mut diagrams = state.ibd_diagrams.lock().map_err(|_| "IBD lock poisoned")?;
    let diagram = diagrams.iter_mut().find(|d| d.id == diagram_id).ok_or("IBD not found")?;
    let property = diagram.properties.iter_mut().find(|p| p.id == property_presentation_id).ok_or("property presentation not found")?;
    let end = ConnectorEnd::nested_port(parse_path(&property.property_path)?, port_id_parsed);
    project.validate_connector_end(parse_element_id(&diagram.context_block_id)?, &end).map_err(|error| error.to_string())?;
    ...
    let (x, y) = match side.as_str() {
        ...
        "right" => (property.x + property.width, property.y + property.height / 2.0),
        "top" => (property.x + property.width / 2.0, property.y),
        "bottom" => (property.x + property.width / 2.0, property.y + property.height),
        _ => return Err("port side must be left, right, top, or bottom".into()),
    };
    property.ports.push(IbdPortPresentation {
        id: id.clone(), element_id: port_id, property_path: property.property_path.clone(), x, y, size: 16.0,
    });
    Ok(id)
}
...
pub fn create_ibd_connector(
    ...
    let semantic_id = project.create_connector(Connector {
    ...
    diagram.connectors.push(IbdConnectorPresentation {
        id: uuid::Uuid::new_v4().to_string(), relationship_id: semantic_id.to_string(),
        source_presentation_id, target_presentation_id, points,
    });
...
pub fn add_item_flow_to_connector(
    ...
    let conveyed: Vec<_> = conveyed_item_ids.iter().map(|id| parse_element_id(id)).collect::<Result<_, _>>()?;
    let mut project_guard = state.project.lock().map_err(|_| "project lock poisoned")?;
    let project = project_guard.as_mut().ok_or("no project open")?;
    let connector = project.relationship(connector_id).map_err(|error| error.to_string())?.connector.clone().ok_or("relationship is not a Connector")?;
    project.create_item_flow(ItemFlow {
        connector_id,
        source: connector.source,
        target: connector.target,
        conveyed_item_ids: conveyed,
    }).map(|id| id.to_string()).map_err(|error| error.to_string())
}
...
pub fn route_ibd(
    ...
    let mut diagrams = state.ibd_diagrams.lock().map_err(|_| "IBD lock poisoned")?;
    let diagram = diagrams.iter_mut().find(|d| d.id == diagram_id).ok_or("IBD not found")?;
    let endpoint_pairs: Vec<_> = diagram.connectors.iter().map(|edge| (edge.source_presentation_id.clone(), edge.target_presentation_id.clone())).collect();
...
pub fn save_ibd_metadata(database: &mut ProjectDatabase, project: &Project, diagrams: &[IbdDiagram]) -> Result<(), String> {
    validate_ibd_diagrams(project, diagrams)?;
    let payload = serde_json::to_string(diagrams).map_err(|error| error.to_string())?;
    database.save_metadata(project.id, IBD_METADATA_KEY, &payload).map_err(|error| error.to_string())
}

pub fn load_ibd_metadata(database: &ProjectDatabase, project: &Project) -> Result<Vec<IbdDiagram>, String> {
    let diagrams = match database.load_metadata(project.id, IBD_METADATA_KEY).map_err(|error| error.to_string())? {
        Some(payload) => serde_json::from_str::<Vec<IbdDiagram>>(&payload).map_err(|error| format!("invalid saved IBD presentation data: {error}"))?,
        None => Vec::new(),
    };
    validate_ibd_diagrams(project, &diagrams)?;
    Ok(diagrams)
