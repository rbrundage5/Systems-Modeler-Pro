...
fn snapshot_complete(project: &Project) -> CompleteProjectSnapshot {
    let mut elements: Vec<_> = project.elements.values().map(|element| CompleteElementSnapshot {
        ...
        flow_direction: element.flow_direction.map(flow_direction_name).map(str::to_string),
    }).collect();
    elements.sort_by(|a, b| a.name.cmp(&b.name));

    let mut relationships: Vec<_> = project.relationships.values().map(|relationship| RelationshipSnapshot {
        ...
        association_ends: relationship.association_ends.iter().map(|end| AssociationEndSnapshot {
            id: end.id.to_string(),
    ...
    CompleteProjectSnapshot { id: project.id.to_string(), name: project.name.clone(), root_id: project.root_id.to_string(), elements, relationships }
}
...
fn validate_complete_diagrams(project: &Project, diagrams: &[BddDiagram]) -> Result<(), String> {
    ...
    for diagram in diagrams {
        ...
        if !diagram_ids.insert(&diagram.id) { return Err(format!("duplicate diagram id: {}", diagram.id)); }
        ...
        for node in &diagram.nodes {
            if uuid::Uuid::parse_str(&node.id).is_err() { return Err(format!("invalid diagram node id: {}", node.id)); }
            if !node_ids.insert(&node.id) { return Err(format!("duplicate diagram node id: {}", node.id)); }
            let element = project.element(parse_element_id(&node.element_id)?).map_err(|error| error.to_string())?;
            if !bdd_presentable(&element.kind) { return Err(format!("element kind {:?} is not valid as a BDD node", element.kind)); }
        }
        for edge in &diagram.edges {
            if uuid::Uuid::parse_str(&edge.id).is_err() { return Err(format!("invalid diagram edge id: {}", edge.id)); }
            if !edge_ids.insert(&edge.id) { return Err(format!("duplicate diagram edge id: {}", edge.id)); }
            ...
            if edge.points.len() < 2 { return Err(format!("diagram edge has no usable route: {}", edge.id)); }
        }
    }
    Ok(())
}
...
#[tauri::command]
pub fn workspace_snapshot_complete(state: tauri::State<'_, WorkspaceState>) -> Result<CompleteWorkspaceSnapshot, String> {
...
#[tauri::command]
pub fn create_bdd_element(kind: String, owner_id: String, name: String, state: tauri::State<'_, WorkspaceState>) -> Result<String, String> {
    let kind = parse_kind(&kind)?;
    if !bdd_presentable(&kind) { return Err(format!("{kind:?} is an owned feature, not a top-level BDD element")); }
...
pub fn create_bdd_feature(
    ...
    let id = match kind {
        ElementKind::EnumerationLiteral | ElementKind::Slot | ElementKind::Operation | ElementKind::Reception => project.create_element(kind, name, owner_id).map_err(|error| error.to_string())?,
        ElementKind::PartProperty | ElementKind::ReferenceProperty | ElementKind::ValueProperty | ElementKind::FlowProperty | ElementKind::ConstraintProperty | ElementKind::ProxyPort | ElementKind::FullPort | ElementKind::Parameter => {
            let type_id = type_id.ok_or_else(|| format!("{kind:?} requires a compatible stable type ID"))?;
            project.create_typed_feature(kind, name, owner_id, parse_element_id(&type_id)?, parse_multiplicity(lower.unwrap_or(1), upper)?).map_err(|error| error.to_string())?
        }
        _ => return Err(format!("{kind:?} is not an owned BDD feature")),
    };
    if let Some(default_value) = default_value { project.element_mut(id).map_err(|error| error.to_string())?.default_value = Some(default_value); }
...
#[tauri::command]
pub fn place_bdd_element(diagram_id: String, element_id: String, x: f64, y: f64, state: tauri::State<'_, WorkspaceState>) -> Result<String, String> {
    ...
    let (width, height) = {
        ...
        if !bdd_presentable(&element.kind) { return Err(format!("{:?} is not valid as a BDD node", element.kind)); }
        match element.kind {
            ElementKind::Enumeration => (190.0, 125.0),
            ElementKind::ConstraintBlock | ElementKind::AssociationBlock => (205.0, 120.0),
            ElementKind::ValueType | ElementKind::DataType | ElementKind::PrimitiveType | ElementKind::Unit | ElementKind::QuantityKind => (185.0, 100.0),
    ...
    if diagram.nodes.iter().any(|node| node.element_id == element_id.to_string()) { return Err("this semantic element is already presented on the BDD".into()); }
    let node_id = uuid::Uuid::new_v4().to_string();
    diagram.nodes.push(DiagramNode { id: node_id.clone(), element_id: element_id.to_string(), x, y, width, height });
    Ok(node_id)
}
...
pub fn create_bdd_relationship_complete(
    ...
    if source_id == target_id { return Err(format!("{kind} cannot connect an element to itself")); }
    let mut project_guard = state.project.lock().map_err(|_| "project lock poisoned")?;
    let project = project_guard.as_mut().ok_or("no project open")?;
    ...
    if !source.is_classifier() || !target.is_classifier() { return Err(format!("{kind} requires classifier endpoints on a BDD")); }
    ...
    let source_node = diagram.nodes.iter().find(|node| node.element_id == source_element_id).cloned().ok_or("source classifier must be presented on the selected BDD")?;
    let target_node = diagram.nodes.iter().find(|node| node.element_id == target_element_id).cloned().ok_or("target classifier must be presented on the selected BDD")?;
    let owner_id = Some(parse_element_id(&diagram.owner_id)?);
    let relationship_id = match kind {
        "Association" | "Aggregation" | "Composition" => {
            let aggregation = match kind { "Aggregation" => AggregationKind::Shared, "Composition" => AggregationKind::Composite, _ => AggregationKind::None };
            project.create_association(owner_id, vec![Project::association_end(source_id, "", Multiplicity::ONE, true, aggregation), Project::association_end(target_id, "", Multiplicity::ONE, true, AggregationKind::None)]).map_err(|error| error.to_string())?
        }
    ...
    diagram.edges.push(DiagramEdge { id: uuid::Uuid::new_v4().to_string(), relationship_id: relationship_id.to_string(), source_node_id: source_node.id, target_node_id: target_node.id, points });
    Ok(relationship_id.to_string())
}

#[tauri::command]
pub fn save_project_file_complete(path: String, state: tauri::State<'_, WorkspaceState>) -> Result<String, String> {
    save_project_file(path, state)
}

#[tauri::command]
pub fn save_current_project_complete(state: tauri::State<'_, WorkspaceState>) -> Result<String, String> {
    save_current_project(state)
}

#[tauri::command]
pub fn open_project_file_complete(path: String, state: tauri::State<'_, WorkspaceState>) -> Result<String, String> {
