...
fn add_ports(project: &Project, diagram: &mut IbdDiagram, index: usize) -> Result<(), String> {
    ...
    let path = property.property_path.iter().map(|id| parse_element_id(id)).collect::<Result<Vec<_>, _>>()?;
    let classifier = project.resolve_structural_path(parse_element_id(&diagram.context_block_id)?, &path)
        .map_err(|error| error.to_string())?;
    for port in project.classifier_features(classifier).map_err(|error| error.to_string())? {
        if !port.is_port() || property.ports.iter().any(|p| p.element_id == port.id.to_string()) {
            continue;
        }
...
pub(super) fn fit_ancestors(diagram: &mut IbdDiagram) -> Result<(), String> {
    ...
    for index in order {
        ...
        let new = super::routing::RouteRect { width, height, ..old };
...
pub(super) fn expand_property(
    ...
    let index = diagram.properties.iter().position(|p| p.id == presentation_id)
    ...
    if parent.property_path.len() >= MAX_PATH_DEPTH {
        return Err(format!("Expansion is limited to {MAX_PATH_DEPTH} levels. Open the type-level IBD to continue; the model is unchanged."));
    }
    let path = parent.property_path.iter().map(|id| parse_element_id(id)).collect::<Result<Vec<_>, _>>()?;
    let classifier = project.resolve_structural_path(parse_element_id(&diagram.context_block_id)?, &path)
        .map_err(|error| error.to_string())?;
    let features = project.classifier_features(classifier).map_err(|error| error.to_string())?;
    let children: Vec<_> = features.into_iter().filter(|feature| {
        matches!(feature.kind, ElementKind::PartProperty | ElementKind::ReferenceProperty)
    }).collect();
    let missing = children.iter().filter(|feature| {
    ...
    if diagram.properties.len() + missing > MAX_OCCURRENCES {
        return Err(format!("This view is limited to {MAX_OCCURRENCES} property occurrences. Open a type-level IBD for further detail."));
    }
    diagram.properties[index].collapsed = false;
    add_ports(project, diagram, index)?;
    let mut y = diagram.properties.iter().filter(|p| is_descendant(&p.property_path, &parent.property_path))
        .map(|p| p.y + p.height + 24.0).fold(parent.y + 48.0, f64::max);
    for feature in children {
        let mut path = parent.property_path.clone();
        path.push(feature.id.to_string());
        if diagram.properties.iter().any(|p| p.property_path == path) { continue; }
...
pub fn set_ibd_structure_expanded(
    ...
    edit_structure(&diagram_id, Some((&presentation_id, expanded)), &state, &activity, &history)
}
...
fn edit_structure(
    ...
    history::apply_structural_specification(workspace, activity, history, |project, diagrams| {
        ...
        if let Some((id, expanded)) = occurrence {
            ...
            for index in 0..diagram.properties.len() { add_ports(project, diagram, index)?; }
            fit_ancestors(diagram)?;
        }
...
mod tests {
    ...
    fn vehicle() -> (Project, IbdDiagram, [ElementId; 4], [ElementId; 3]) {
        ...
        let package = project.create_element(ElementKind::Package, "VehicleDefinitions", project.root_id).unwrap();
        let blocks = ["Vehicle", "Engine", "Piston", "Ring"].map(|name| {
            project.create_element(ElementKind::Block, name, package).unwrap()
        });
        let mut parts = Vec::new();
        for (index, (name, multiplicity)) in [("engine", 1), ("pistons", 4), ("rings", 3)].into_iter().enumerate() {
            parts.push(project.create_typed_feature(ElementKind::PartProperty, name, blocks[index], blocks[index + 1], Multiplicity::new(multiplicity, Some(multiplicity)).unwrap()).unwrap());
        }
    ...
    fn vehicle_expansion_reuses_definitions_and_persists_contextual_occurrences() {
        ...
        let second = project.create_typed_feature(ElementKind::PartProperty, "rearEngine", blocks[0], blocks[1], Multiplicity::ONE).unwrap();
        ibd::populate_ibd_diagram_from_context(&project, &mut diagram).unwrap();
        let original = serde_json::to_value(&project).unwrap();
        let engines: Vec<_> = diagram.properties.iter().map(|p| p.id.clone()).collect();
        for id in &engines { expand_property(&project, &mut diagram, id, true).unwrap(); }
        let pistons: Vec<_> = diagram.properties.iter().filter(|p| p.element_id == parts[1].to_string()).map(|p| p.id.clone()).collect();
        assert_eq!(pistons.len(), 2);
        for id in &pistons { expand_property(&project, &mut diagram, id, true).unwrap(); }
        assert_eq!(diagram.properties.len(), 6);
        assert!(diagram.properties.iter().any(|p| p.property_path == vec![second.to_string(), parts[1].to_string(), parts[2].to_string()]));
        ...
        assert_eq!(diagram.properties.iter().filter(|p| property_visible(&diagram, p)).count(), 4);
        ...
        for (index, part) in parts.iter().enumerate() {
            assert_eq!(reopened.element(*part).unwrap().owner_id, Some(blocks[index]));
            assert_eq!(reopened.element(*part).unwrap().type_id, Some(blocks[index + 1]));
        }
    }
    ...
    fn expansion_is_one_transaction_with_noop_rejection_and_legacy_defaults() {
        ...
        legacy["properties"][0].as_object_mut().unwrap().remove("collapsed");
        ...
        *workspace.ibd_diagrams.lock().unwrap() = vec![legacy];
        ...
        assert!(edit_structure(&id, Some(("missing", true)), &workspace, &activity, &history).is_err());
        assert!(history::redo_states(&workspace, &activity, &history).unwrap());
        assert_eq!(workspace.ibd_diagrams.lock().unwrap()[0].properties.len(), 2);
    }

    #[test]
    fn recursive_types_allow_bounded_lazy_expansion_without_semantic_cycles() {
        let (mut project, mut diagram, blocks, _) = vehicle();
        project.create_typed_feature(ElementKind::PartProperty, "nested", blocks[1], blocks[1], Multiplicity::new(0, Some(1)).unwrap()).unwrap();
        ...
        for _ in 1..MAX_PATH_DEPTH {
            ...
            current = diagram.properties.iter().filter(|p| project.element(parse_element_id(&p.element_id).unwrap()).unwrap().name == "nested").max_by_key(|p| p.property_path.len()).unwrap().id.clone();
        }
