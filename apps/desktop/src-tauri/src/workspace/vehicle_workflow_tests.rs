...
use systems_modeler_core::{Multiplicity, Project, StructuralRuntime, StructuralRuntimeConfiguration};
...
fn vehicle_bdd_authoring_nested_ibd_persistence_and_deletion_share_identity() {
    ...
    state.diagrams.lock().unwrap().push(BddDiagram {
        id: view.clone(), name: "Vehicle decomposition".into(), owner_id: package.to_string(),
        family: "bdd".into(), semantic_context_id: None, subject_boundary: None,
        nodes: Vec::new(), edges: Vec::new(),
    });
    *state.project.lock().unwrap() = Some(project);
    for (i, (name, multiplicity)) in [("engine", "1"), ("pistons", "4"), ("rings", "3")].into_iter().enumerate() {
        author_in_state(&view, &PartCompositionRequest {
            owner_id: blocks[i].to_string(), type_id: blocks[i + 1].to_string(),
            property_id: None, name: Some(name.into()), multiplicity: Some(multiplicity.into()),
        }, &state, &activity, &history).unwrap();
    }
    let mut project = state.project.lock().unwrap().as_ref().unwrap().clone();
    let parts: Vec<_> = blocks[..3].iter().map(|owner| project.owned_features(*owner).next().unwrap().id).collect();
    ...
    for block in blocks { assert_eq!(project.element(block).unwrap().owner_id, Some(package)); }
    assert_eq!(state.diagrams.lock().unwrap()[0].nodes.len(), 4);
    assert_eq!(state.diagrams.lock().unwrap()[0].edges.len(), 3);
    let runtime = StructuralRuntime::build(&project, blocks[0], &StructuralRuntimeConfiguration::default()).unwrap();
    ...
    let mut diagram = ibd::IbdDiagram {
        id: uuid::Uuid::new_v4().to_string(), name: "Vehicle internal structure".into(),
        context_block_id: blocks[0].to_string(), owner_id: package.to_string(),
    ...
    for depth in 1..=2 {
        let id = diagram.properties.iter().find(|p| p.property_path.len() == depth).unwrap().id.clone();
    ...
    project.set_multiplicity(parts[1], Multiplicity::new(6, Some(6)).unwrap()).unwrap();
    let end = project.relationships.values().flat_map(|r| &r.association_ends)
        .find(|end| end.property_id == Some(parts[1])).unwrap();
    ...
    db.save_project_with_metadata(&project, &[(crate::workspace::BDD_METADATA_KEY, &bdd), (ibd::IBD_METADATA_KEY, &ibd_payload)]).unwrap();
    drop(db);
    ...
    assert_eq!(db.load_metadata(project.id, crate::workspace::BDD_METADATA_KEY).unwrap().unwrap(), bdd);
    assert!(ibd_structure::remove_property_presentation(&mut diagram, &root));
    ...
    *state.ibd_diagrams.lock().unwrap() = saved;
    repository_editing::delete_model_element_in_state(parts[0], &state, &activity, &history).unwrap();
    {
        let guard = state.project.lock().unwrap();
        let project = guard.as_ref().unwrap();
        assert!(project.element(parts[0]).is_err());
        for block in blocks { assert!(project.element(block).is_ok()); }
    }
    assert!(history::undo_states(&state, &activity, &history).unwrap());
    assert!(state.project.lock().unwrap().as_ref().unwrap().element(parts[0]).is_ok());
