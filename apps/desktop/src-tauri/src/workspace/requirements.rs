...
pub fn create_traceability_relationship(
    ...
    let diagram = diagrams
        ...
        .find(|candidate| candidate.id == diagram_id && candidate.family == "requirement")
        .ok_or("Requirement Diagram not found")?;
    let source = diagram.nodes.iter().find(|node| node.id == source_node_id).ok_or("source node not found")?;
    let target = diagram.nodes.iter().find(|node| node.id == target_node_id).ok_or("target node not found")?;
    ...
    let points = vec![
        DiagramPoint { x: source.x + source.width / 2.0, y: source.y + source.height },
        DiagramPoint { x: target.x + target.width / 2.0, y: target.y },
    ];
