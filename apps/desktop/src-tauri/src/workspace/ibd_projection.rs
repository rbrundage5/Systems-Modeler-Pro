...
pub(super) fn project_end(prefix: &[ElementId], end: &ConnectorEnd) -> ConnectorEnd {
    if prefix.is_empty() { return end.clone(); }
    let mut path = prefix.to_vec();
    path.extend(&end.property_path);
    ConnectorEnd {
        role_id: if end.port_id.is_some() { *path.last().expect("nonempty prefix") } else { end.role_id },
        property_path: path,
        port_id: end.port_id,
    }
}
...
fn presented_end(diagram: &IbdDiagram, end: &ConnectorEnd) -> Option<String> {
    diagram.boundary_ports.iter().map(|port| port.id.clone())
        .chain(diagram.properties.iter().flat_map(|p| std::iter::once(p.id.clone()).chain(p.ports.iter().map(|port| port.id.clone()))))
        .find(|id| ibd::ibd_end_for_presentation(diagram, id).is_ok_and(|(candidate, _)| candidate == *end))
}

pub(super) fn show_existing_connectors(project: &Project, diagram: &mut IbdDiagram) -> Result<(), String> {
    let root = parse_element_id(&diagram.context_block_id)?;
    let mut contexts = vec![Vec::new()];
    contexts.extend(diagram.properties.iter().filter(|p| !p.collapsed && super::ibd_structure::property_visible(diagram, p))
        .map(|p| p.property_path.clone()));
    for context_path in contexts {
        let prefix = context_path.iter().map(|id| parse_element_id(id)).collect::<Result<Vec<_>, _>>()?;
        let classifier = project.resolve_structural_path(root, &prefix).map_err(|error| error.to_string())?;
        for relationship in project.relationships.values() {
            let Some(connector) = &relationship.connector else { continue; };
            if connector.context_id != classifier || diagram.connectors.iter().any(|edge| edge.relationship_id == relationship.id.to_string() && edge.context_path == context_path) {
                continue;
            }
            let source = project_end(&prefix, &connector.source);
            let target = project_end(&prefix, &connector.target);
            let (Some(source_id), Some(target_id)) = (presented_end(diagram, &source), presented_end(diagram, &target)) else { continue; };
...
mod tests {
    ...
    fn repeated_type_usages_project_delegation_without_creating_relationships() {
        ...
        let child = project.create_typed_feature(ElementKind::PartProperty, "child", component, component, Multiplicity::new(0, Some(1)).unwrap()).unwrap();
        let connector_id = project.create_connector(Connector {
        ...
        project.create_typed_feature(ElementKind::PartProperty, "second", context, component, Multiplicity::ONE).unwrap();
        ...
        for id in &ids { super::super::ibd_structure::expand_property(&project, &mut diagram, id, true).unwrap(); }
        let occurrences: Vec<_> = diagram.connectors.iter().filter(|edge| edge.relationship_id == connector_id.to_string()).collect();
