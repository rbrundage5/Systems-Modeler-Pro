...
use super::{
    ...
    bulk_model::{
        ...
        ConnectorEndBuildSpec, ElementReference, ModelBuildOperation, ModelBuildPlan, ModelBuildResult,
...
fn parse_connector_kind(
    ...
    let value = non_empty_value(values, SpreadsheetSemanticProperty::ConnectorKind).ok_or_else(
        || {
            diagnostic(
                Some(map),
                Some(row),
...
fn resolved_existing_connector_end(
    ...
    super::bulk_model::resolve_connector_end(
        project,
        &map.source_namespace,
        context_id,
        spec,
        0,
    )
...
fn plan_connector_row(
    ...
    if !seen_source_external_ids.insert(key.clone()) {
        return Err(diagnostic(
            ...
            format!("source Connector identity '{key}' appears more than once in this import group"),
        ));
    }
        ...
        match matches.as_slice() {
            ...
            _ => {
                return Err(diagnostic(
                    ...
                    Some(effective_external_id),
                    "AMBIGUOUS_RELATIONSHIP",
                    format!("full Connector topology matches {} relationships", matches.len()),
                ));
            }
        }
