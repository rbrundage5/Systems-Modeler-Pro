...
fn reroute_diagram(
    ...
    for (index, presentation) in diagram.edges.iter_mut().enumerate() {
        ...
        // Activity flows receive a diagram-wide deterministic lane rather than
        // only separating duplicate source/target pairs. This keeps decision,
        // merge, fork, and join branches from collapsing onto the same dogleg
        // corridor while retaining the shared obstacle-safe router.
...
pub fn delete_activity_item(
    ...
    let original_activity = repository.activities.get(&activity_id).cloned().ok_or("Activity not found")?;

    {
        let activity = repository.activities.get_mut(&activity_id).ok_or("Activity not found")?;
        match item_kind.as_str() {
            "edge" => {
                ...
                if before == activity.edges.len() { return Err("Activity edge not found".into()); }
                diagram.edges.retain(|edge| edge.activity_edge_id != edge_id.to_string());
            }
            "node" => {
                let node_id = activity_workspace::parse_activity_node_id(&item_id)?;
                if !activity.nodes.iter().any(|node| node.id == node_id) { return Err("Activity node not found".into()); }
                let pins = pin_ids_for_node(activity, node_id);
                let incident: HashSet<_> = activity.edges.iter().filter(|edge| endpoint_is_incident(edge.source, node_id, &pins) || endpoint_is_incident(edge.target, node_id, &pins)).map(|edge| edge.id).collect();
                let incident_strings: HashSet<_> = incident.iter().map(ToString::to_string).collect();
                activity.edges.retain(|edge| !incident.contains(&edge.id));
                activity.nodes.retain(|node| node.id != node_id);
                let removed_presentations: HashSet<_> = diagram.nodes.iter().filter(|node| node.activity_node_id == node_id.to_string()).map(|node| node.id.clone()).collect();
                diagram.nodes.retain(|node| node.activity_node_id != node_id.to_string());
                diagram.edges.retain(|edge| !incident_strings.contains(&edge.activity_edge_id) && !removed_presentations.contains(&edge.source_node_id) && !removed_presentations.contains(&edge.target_node_id));
            }
    ...
    let activity = repository.activities.get(&activity_id).ok_or("Activity not found")?;
...
pub fn reconnect_activity_edge(
    ...
    let (source, target) = {
        ...
        if source == target || endpoint_owner(activity, source)? == endpoint_owner(activity, target)? { return Err("Activity flow requires distinct source and target nodes".into()); }
        (source, target)
    };
    {
        let activity = repository.activities.get_mut(&activity_id).ok_or("Activity not found")?;
        let edge = activity.edges.iter_mut().find(|edge| edge.id == edge_id).ok_or("Activity edge not found")?;
        edge.source = source;
        edge.target = target;
    }
    ...
    let source_presentation_id = diagram.nodes.iter().find(|node| node.activity_node_id == source_owner).ok_or("source Activity endpoint owner is not presented on this diagram")?.id.clone();
    let target_presentation_id = diagram.nodes.iter().find(|node| node.activity_node_id == target_owner).ok_or("target Activity endpoint owner is not presented on this diagram")?.id.clone();
    let presentation = diagram.edges.iter_mut().find(|edge| edge.activity_edge_id == edge_id.to_string()).ok_or("Activity edge is not presented on this diagram")?;
...
pub fn route_activity_diagram(
    ...
    let diagram = diagrams.iter_mut().find(|diagram| diagram.id == diagram_id).ok_or("Activity diagram not found")?;
    let activity_id = activity_workspace::parse_activity_id(&diagram.activity_id)?;
    let repository = activity_state.repository.lock().map_err(|_| "Activity repository lock poisoned")?;
    let activity = repository.activities.get(&activity_id).ok_or("Activity not found")?;
    reroute_diagram(diagram, activity)
}
