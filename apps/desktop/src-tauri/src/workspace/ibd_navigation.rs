...
fn open_type_ibd(
    ...
    history::apply_structural_specification(workspace, activity, history, |project, diagrams| {
        let selected = project.element(element_id).map_err(|error| error.to_string())?;
        let context = if matches!(selected.kind, ElementKind::PartProperty | ElementKind::ReferenceProperty) {
            project.element(selected.type_id.ok_or("The selected property has no type")?).map_err(|error| error.to_string())?
        } else { selected };
        ...
        if let Some(existing) = diagrams.iter().find(|d| d.context_block_id == context.id.to_string()) {
        ...
        loop {
            ...
            if matches!(owner.kind, ElementKind::Model | ElementKind::Package) { break; }
        ...
        for index in 0..diagram.properties.len() { super::ibd_structure::add_ports(project, &mut diagram, index)?; }
...
mod tests {
    ...
    fn type_navigation_reuses_definition_and_diagram_with_one_history_entry() {
        ...
        let system = project.create_element(ElementKind::Block, "Vehicle", root).unwrap();
        let engine = project.create_element(ElementKind::Block, "Engine", root).unwrap();
        let part = project.create_typed_feature(ElementKind::PartProperty, "engine", system, engine, Multiplicity::ONE).unwrap();
        ...
        *workspace.project.lock().unwrap() = Some(project);
        ...
        assert_eq!(workspace.ibd_diagrams.lock().unwrap()[0].context_block_id, engine.to_string());
        assert_eq!(history::undo_len(&history), 1);
        assert_eq!(open_type_ibd(engine, &workspace, &activity, &history).unwrap(), id);
        assert_eq!(history::undo_len(&history), 1);
        ...
        assert_eq!(serde_json::to_value(workspace.project.lock().unwrap().as_ref().unwrap()).unwrap(), original);
    }
}
