(() => {
  ...
  renderProperties = function renderPropertiesWithFeatureSemantics() {
    ...
    panel.innerHTML = `<div class="property-heading">${escapeHtml(element.kind)}</div>
      ...
      ${supportsAggregation ? `<label>Aggregation<select id="property-aggregation">${element.kind === 'PartProperty' ? '<option value="composite">composite</option>' : '<option value="none">none</option><option value="shared">shared</option>'}</select></label>` : ''}
