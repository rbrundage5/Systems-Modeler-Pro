...
paletteSymbol = function paletteSymbolComplete(item) {
  const symbols = {
    ...
    PartProperty: '◆', ReferenceProperty: '◇', ValueProperty: 'v', ConstraintProperty: 'c',
    ProxyPort: '□', FullPort: '■', Operation: 'ƒ', Reception: '⇥', Parameter: 'p', EnumerationLiteral: '•',
...
renderPalette = function renderPaletteComplete() {
  ...
  for (const [category, title] of groups) {
    ...
    for (const item of items) {
      ...
      const active = category === 'element'
        ...
        : category === 'relationship' && state.pendingRelationship?.kind === item.relationship_kind;
      ...
      if (item.draggable) {
        ...
        button.title = 'Select a compatible semantic owner, then click this feature.';
      }
  ...
  hint.textContent = 'Classifier and relationship eligibility comes from Rust. Owned features are created under the selected classifier or operation and rendered in compartments.';
...
async function chooseTypeId(kind) {
  ...
  const compatible = {
    PartProperty: ['Block', 'InterfaceBlock'],
    ReferenceProperty: ['Block', 'InterfaceBlock', 'DataType', 'ValueType', 'Enumeration'],
    ValueProperty: ['ValueType', 'DataType', 'Enumeration'],
    ConstraintProperty: ['ConstraintBlock'],
    ProxyPort: ['InterfaceBlock', 'Block', 'DataType'],
    FullPort: ['InterfaceBlock', 'Block', 'DataType'],
...
async function createFeatureFromPalette(item) {
  ...
  if (BDD_TYPED_FEATURE_KINDS.has(item.semantic_kind)) {
    const notation = prompt('Multiplicity', '1');
    if (!notation) return;
    if (notation === '*') { lower = 0; upper = null; }
    else if (notation.includes('..')) {
      const [lo, hi] = notation.split('..');
      lower = Number(lo);
      upper = hi === '*' ? null : Number(hi);
    } else {
      lower = Number(notation);
      upper = Number(notation);
    }
  ...
  state.selectedElementId = featureId;
  await refresh();
}
...
activatePaletteItem = function activatePaletteItemComplete(item) {
  ...
  if (item.category === 'relationship') {
    ...
    state.paletteTool = null;
    createFeatureFromPalette(item).catch((error) => {
      console.error(error);
...
createPaletteElementAt = async function createPaletteElementAtComplete(item, x, y) {
  ...
  const name = prompt(`${item.label} name`, `New ${item.label}`);
  if (!name) return;
  const elementId = await runCommand(`Creating ${item.label}…`, () => requireInvoke()('create_bdd_element', {
    kind: item.semantic_kind,
    ownerId: diagram.owner_id,
    name,
  }));
  await runCommand(`Placing ${item.label}…`, () => requireInvoke()('place_bdd_element', {
...
placeExistingElementAt = async function placeExistingElementAtComplete(elementId, x, y) {
  await runCommand('Placing existing classifier…', () => requireInvoke()('place_bdd_element', {
...
refresh = async function refreshComplete() {
  ...
  if (state.selectedDiagramId && !state.snapshot?.diagrams?.some((diagram) => diagram.id === state.selectedDiagramId)) state.selectedDiagramId = null;
  await loadPalette();
  render();
};
...
renderRepository = function renderRepositoryComplete() {
  ...
  function appendChildren(ownerId, depth) {
    for (const element of (byOwner.get(ownerId) || []).sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (BDD_CLASSIFIER_KINDS.has(element.kind)) {
        row.draggable = true;
        row.title = 'Drag onto a BDD to create another presentation of this existing semantic classifier.';
      }
      row.onclick = () => {
        state.selectedRelationshipId = null;
        state.paletteTool = null;
      ...
      appendChildren(element.id, depth + 1);
    }
  }
...
renderProperties = function renderPropertiesComplete() {
  ...
  panel.innerHTML = `<div class="property-heading">${escapeHtml(element.kind)}</div>
    ...
    ${element.multiplicity ? `<label>Multiplicity<input value="${escapeAttr(element.multiplicity)}" disabled></label>` : ''}
...
async function saveProjectAsComplete() {
  ...
  if (!path) return;
  await runCommand('Saving project…', () => requireInvoke()('save_project_file_complete', { path }));
  await refresh();
}

async function saveProjectComplete() {
  if (!state.snapshot?.project) return alert('Create or open a project first.');
  if (!state.snapshot.current_file) return saveProjectAsComplete();
  await runCommand('Saving project…', () => requireInvoke()('save_current_project_complete'));
  await refresh();
}
