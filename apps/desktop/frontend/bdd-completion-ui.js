...
const renderStructuralProperties = renderProperties; renderProperties = function renderPropertiesComplete() {
  ...
  $('apply-element').onclick = async () => {
    ...
    await runCommand('Applying element specification…', () => requireInvoke()('update_element_specification', { elementId: element.id, edit: { name, documentation: $('property-documentation').value, defaultValue: $('property-default')?.value ?? null, quantityKindExternalId: $('property-quantity-kind')?.value ?? null, unitExternalId: $('property-unit')?.value ?? null } }));
    await refresh();
  };
};
