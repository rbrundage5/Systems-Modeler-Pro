...
use systems_modeler_persistence::collaboration::{
    PresenceParticipant, PresenceRequest, ProjectPresence,
};
use uuid::Uuid;
...
impl PresenceRegistry {
    fn expire(&mut self, now: Instant) {
        self.sessions
            .retain(|_, value| now.saturating_duration_since(value.last_seen) < TTL);
    }
    ...
    pub(crate) fn heartbeat(
        ...
        now: Instant,
    ) -> Result<ProjectPresence, u16> {
        let name = request.name.trim();
        if request.session.is_nil()
            || name.is_empty()
            || name.len() > 256
            || name.chars().count() > 64
    ...
    pub(crate) fn snapshot(&mut self, project: ProjectId, now: Instant) -> ProjectPresence {
        ...
        let mut participants: Vec<_> = self
            .sessions
            .iter()
            .filter(|((id, _, _), _)| *id == project)
            .map(|(_, value)| value.participant.clone())
            .collect();
...
mod tests {
    ...
    fn presence_is_scoped_expires_and_cannot_be_removed_by_another_actor() {
        ...
        assert!(
            registry
                .snapshot(Project::new("Other").id, now)
                .participants
                .is_empty()
        );
        assert_eq!(
            registry
                .leave(project, Uuid::new_v4(), session, now)
                .participants
                .len(),
            1
        );
        assert_eq!(
            registry
                .snapshot(project, now + TTL - Duration::from_millis(1))
                .participants
                .len(),
            1
        );
    ...
    fn presence_is_bounded_and_existing_sessions_can_renew_at_capacity() {
        ...
        assert_eq!(
            registry
                .leave(project, actor, session, now)
                .participants
                .len(),
            MAX_ACTOR_SESSIONS - 1
        );
        assert!(
            registry
                .heartbeat(
