...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.get(1).map(String::as_str) {
        Some("credential") if args.len() == 2 => {
            ...
            println!(
                "{}",
                serde_json::json!({"actor":Uuid::new_v4(),"token":token,"token_sha256":token_hash(&token)})
            );
        }
        ...
        _ => return Err(
            "usage: systems-modeler-server credential | init DATABASE NAME | serve DATABASE CONFIG"
                .into(),
        ),
    }
    Ok(())
