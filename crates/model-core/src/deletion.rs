...
impl Project {
    pub fn stage_connected_element_deletion(
        &self,
        id: ElementId,
    ) -> Result<ElementDeletion, String> {
        if id == self.root_id {
        ...
        loop {
            let owned: Vec<_> = self
                ...
                .filter(|element| {
                    element
                        .owner_id
                        .is_some_and(|owner| elements.contains(&owner))
                })
        ...
        let mut relationships: HashSet<_> = self
            .relationships
            .values()
            .filter(|relationship| {
                elements.contains(&relationship.source_id)
                    ...
                    || relationship
                        .owner_id
                        .is_some_and(|id| elements.contains(&id))
                    ...
                    || relationship.connector.as_ref().is_some_and(|connector| {
                        elements.contains(&connector.context_id)
                            || endpoint_removed(&connector.source)
                            || endpoint_removed(&connector.target)
                    })
                    || relationship.item_flow.as_ref().is_some_and(|flow| {
                        endpoint_removed(&flow.source)
                            || endpoint_removed(&flow.target)
                            || flow
                                .conveyed_item_ids
                                .iter()
        ...
        let flows: Vec<_> = self
            ...
            .filter(|relationship| {
                relationship
                    .item_flow
                    .as_ref()
                    .is_some_and(|flow| relationships.contains(&flow.connector_id))
            })
            .map(|relationship| relationship.id)
            .collect();
        ...
        project
            .relationships
            .retain(|id, _| !relationships.contains(id));
        project.elements.retain(|id, _| !elements.contains(id));
        project
            .profiles
            .profile_applications
            .retain(|_, application| !elements.contains(&application.scope_id));
        project
            .profiles
            .stereotype_applications
            .retain(|_, application| match application.target {
        ...
        project.validate().map_err(|error| {
            format!("Cannot delete while a surviving dependency would be invalid: {error}")
        })?;
        Ok(ElementDeletion {
            project,
            elements,
