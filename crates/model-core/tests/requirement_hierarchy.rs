...
fn nested_requirements_preserve_identity_through_rename_and_reparent() {
    ...
    let system = project
        .create_requirement("System", "R-1", "System text", root)
        .unwrap();
    let subsystem = project
        .create_requirement("Subsystem", "R-2", "Subsystem text", system)
        .unwrap();
    let component = project
        .create_requirement("Component", "R-3", "Component text", subsystem)
        .unwrap();
    let id = project.element(component).unwrap().external_id.clone();
    project.validate().unwrap();
    assert_eq!(
        project.qualified_name(component).unwrap(),
        "System::System::Subsystem::Component"
    );
...
fn illegal_requirement_ownership_and_cycles_preserve_the_model() {
    ...
    assert!(
        project
            .create_element(ElementKind::Block, "Invalid", parent)
            .is_err()
    );
    assert!(
        project
            .create_requirement("Duplicate", "R-2", "Invalid", parent)
            .is_err()
    );
...
fn deleting_a_compound_requirement_retires_descendants_and_incident_traceability() {
    ...
    let leaf = project
        ...
        .unwrap();
    let block = project
        .create_element(ElementKind::Block, "Implementation", root)
        .unwrap();
    let link = project
