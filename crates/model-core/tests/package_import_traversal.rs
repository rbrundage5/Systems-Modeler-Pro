...
fn package(project: &mut Project, name: &str) -> ElementId {
    project
        .create_element(ElementKind::Package, name, project.root_id)
        .unwrap()
}
...
fn repeated_diamonds_visit_each_export_once_without_expanding_every_path() {
    ...
    let member = project
        .create_element(ElementKind::Signal, "Payload", library)
        .unwrap();
    for source in previous {
        import(&mut project, source, library);
        project
            .create_element_import(source, member, VisibilityKind::Public, Some("Data".into()))
            .unwrap();
    }
...
fn chained_imports_do_not_use_the_call_stack_or_rescan_each_library() {
    ...
    let member = project
        .create_element(ElementKind::Signal, "Payload", libraries[20_000])
        .unwrap();
    assert_eq!(
        project.resolve_name(libraries[0], "Payload").unwrap(),
        member
    );
...
fn visibility_alias_ambiguity_and_owned_precedence_survive_fresh_queries() {
    ...
    let member = project
        .create_element(ElementKind::Signal, "Payload", library)
        .unwrap();
    let other_member = project
        .create_element(ElementKind::Signal, "Payload", other)
        .unwrap();
    import(&mut project, consumer, middle);
    let edge = project
        .create_package_import(middle, library, VisibilityKind::Private)
        .unwrap();
    assert_eq!(project.resolve_name(middle, "Payload").unwrap(), member);
    assert!(matches!(
        project.resolve_name(consumer, "Payload"),
    ...
    project
        .create_element_import(middle, member, VisibilityKind::Public, Some("Alias".into()))
        .unwrap();
    project
        ...
        .unwrap();
    assert!(
        matches!(project.resolve_name(consumer, "Alias"), Err(NamespaceResolutionError::Ambiguous { candidates, .. }) if candidates.len() == 2)
    );
    let local = project
        .create_element(ElementKind::Signal, "Alias", consumer)
        .unwrap();
