...
fn generalize(project: &mut Project, child: ElementId, parent: ElementId) {
    project.create_relationship(RelationshipKind::Generalization, child, parent, Some(project.root_id)).unwrap();
}

fn typed(project: &mut Project, kind: ElementKind, name: &str, owner: ElementId, type_id: ElementId) -> ElementId {
    project.create_typed_feature(kind, name, owner, type_id, Multiplicity::ONE).unwrap()
}
...
fn inherited_parts_and_ports_form_valid_delegation_without_copying_features() {
    ...
    let interface = project.create_element(ElementKind::InterfaceBlock, "Interface", project.root_id).unwrap();
    generalize(&mut project, child, base);
    generalize(&mut project, component, component_base);
    let part = typed(&mut project, ElementKind::PartProperty, "component", base, component);
    let boundary = typed(&mut project, ElementKind::ProxyPort, "external", base, interface);
    let port = typed(&mut project, ElementKind::ProxyPort, "internal", component_base, interface);
    assert_eq!(project.resolve_structural_path(child, &[part]).unwrap(), component);
    ...
    assert_eq!(reopened.element(port).unwrap().owner_id, Some(component_base));
    assert_eq!(reopened.relationship(id).unwrap().connector.as_ref().unwrap().target.port_id, Some(port));
}
...
fn private_and_unrelated_features_reject_without_mutating_relationships() {
    ...
    let component = block(&mut project, "Component");
    generalize(&mut project, child, base);
    let part = typed(&mut project, ElementKind::PartProperty, "part", base, component);
    let other = typed(&mut project, ElementKind::PartProperty, "other", child, component);
    ...
    for context in [child, unrelated] {
        ...
        assert_eq!(project.resolve_structural_path(context, &[part]), Err(ModelError::InvalidConnectorPath(part)));
        assert!(project.create_connector(Connector {
            context_id: context,
...
fn diamond_features_keep_original_identity_and_private_features_remain_local() {
    ...
    let operation = project.create_element(ElementKind::Operation, "run", base).unwrap();
    let hidden = project.create_element(ElementKind::Operation, "hidden", base).unwrap();
    project.elements.get_mut(&hidden).unwrap().visibility = VisibilityKind::Private;
    let local = project.create_element(ElementKind::Operation, "hidden", child).unwrap();
    project.elements.get_mut(&local).unwrap().visibility = VisibilityKind::Private;
    let before = serde_json::to_value(&project).unwrap();
    let ids: Vec<_> = project.classifier_features(child).unwrap().iter().map(|feature| feature.id).collect();
    assert_eq!(ids.len(), 2);
