...
fn diamond_emits_each_ancestor_feature_once_and_preserves_distinct_same_named_features() {
    ...
    for owner in [left, right, ancestor] {
        expected.push(
            project
                .create_element(ElementKind::Operation, "operate", owner)
                .unwrap(),
        );
    }
    let own = project
        .create_element(ElementKind::Operation, "local", child)
        .unwrap();
...
fn repeated_edges_and_malformed_cycles_terminate_without_duplicate_or_local_features() {
    ...
    let feature = project
        .create_element(ElementKind::Operation, "inherited", parent)
        .unwrap();
    project
        .create_element(ElementKind::Operation, "local", child)
        .unwrap();
...
fn deep_feature_query_does_not_recurse_or_scan_the_entire_repository_per_ancestor() {
    ...
    let nodes: Vec<_> = (0..20_001)
        .map(|index| block(&mut project, &format!("Type {index}")))
        .collect();
    ...
    let feature = project
        .create_element(ElementKind::Operation, "deep feature", nodes[20_000])
        .unwrap();
...
fn missing_and_non_classifier_queries_preserve_existing_errors() {
    ...
    assert!(
        matches!(project.inherited_features(missing), Err(ModelError::ElementNotFound(id)) if id == missing)
    );
    assert!(matches!(
        project.inherited_features(project.root_id),
