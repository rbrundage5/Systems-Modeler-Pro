...
fn end(classifier: ElementId) -> AssociationEnd {
    Project::association_end(classifier, "role", Multiplicity::ONE, true, AggregationKind::None)
}
...
fn invalid_end_payloads_are_rejected_before_relationship_publication() {
    ...
    multiplicity[1].multiplicity = Multiplicity { lower: 3, upper: Some(1) };
    ...
    for ends in [
        ...
        assert!(project.create_association(Some(project.root_id), ends).is_err());
...
fn reopened_associations_cannot_disagree_with_their_endpoints_or_end_types() {
    let (mut project, [a, b, c]) = fixture();
    let id = project.create_association(Some(project.root_id), vec![end(a), end(b)]).unwrap();
    ...
    for invalid in 0..5 {
        ...
        match invalid {
            ...
            3 => association.association_ends[0].multiplicity = Multiplicity { lower: 2, upper: Some(0) },
...
fn member_end_identity_is_unique_across_distinct_associations() {
    ...
    let id = project.create_association(Some(project.root_id), ends.clone()).unwrap();
    let before = serde_json::to_value(&project).unwrap();
    assert!(matches!(
    ...
    assert!(matches!(project.validate(), Err(ModelError::DuplicateAssociationEndId(_))));
}

#[test]
fn parallel_reflexive_nary_and_legacy_associations_remain_readable() {
    let (mut project, [a, b, c]) = fixture();
    for ends in [vec![end(a), end(b)], vec![end(a), end(b)], vec![end(a), end(a)], vec![end(a), end(b), end(c)]] {
        project.create_association(Some(project.root_id), ends).unwrap();
    }
    ...
    project.create_relationship(RelationshipKind::Association, a, b, Some(project.root_id)).unwrap();
    project.create_relationship(RelationshipKind::Composition, a, b, Some(project.root_id)).unwrap();
