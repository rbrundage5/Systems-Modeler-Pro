...
fn runtime_realizes_only_links_consistent_with_explicit_connector_end_bounds() {
    use systems_modeler_core::{StructuralRuntime, StructuralRuntimeConfiguration, StructuralRuntimeError};
    ...
    assert_eq!(runtime.connector_links.iter().map(|link| link.id).collect::<Vec<_>>(), repeated.connector_links.iter().map(|link| link.id).collect::<Vec<_>>());
    assert_eq!(serde_json::to_value(&project).unwrap(), before);
    project.relationships.get_mut(&id).unwrap().connector.as_mut().unwrap().end_multiplicities[0] = Multiplicity::ONE;
    ...
    assert!(matches!(error, StructuralRuntimeError::ConnectorEndCardinality { count: 4, .. }));
...
fn optional_absent_typed_endpoint_does_not_fabricate_runtime_links() {
    ...
    let (mut project, mut connector) = fixture();
    let context = connector.context_id;
    project.element_mut(connector.source.role_id).unwrap().multiplicity = Some(Multiplicity::new(0, Some(0)).unwrap());
    connector.end_multiplicities = [Multiplicity::new(0, Some(1)).unwrap(), Multiplicity::ONE];
    project.create_connector(connector).unwrap();
    let runtime = StructuralRuntime::build(&project, context, &StructuralRuntimeConfiguration::default()).unwrap();
