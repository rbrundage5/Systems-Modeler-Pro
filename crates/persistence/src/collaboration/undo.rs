...
fn changes<K: Copy + Eq + Hash, T: Clone + Serialize>(before: &HashMap<K, T>, after: &HashMap<K, T>) -> Result<Vec<Change<K, T>>, CollaborationError> {
    ...
    for id in keys {
        if serde_json::to_value(before.get(&id))? != serde_json::to_value(after.get(&id))? {
            result.push(Change { id, before: before.get(&id).cloned(), after: after.get(&id).cloned() });
        }
    }
    Ok(result)
}

fn reverse<K: Copy + Eq + Hash, T: Clone + Serialize>(records: &mut HashMap<K, T>, changes: &[Change<K, T>]) -> Result<(), CollaborationError> {
...
impl UndoDelta {
    pub(super) fn capture(before: &Project, after: &Project, before_diagrams: &[SharedBddDiagram], after_diagrams: &[SharedBddDiagram]) -> Result<Self, CollaborationError> {
        ...
        if before.id != after.id || before.root_id != after.root_id || before.name != after.name
        ...
        Ok(Self {
            ...
            diagrams: changes(&before_diagrams.iter().map(|value| (value.id, value.clone())).collect(), &after_diagrams.iter().map(|value| (value.id, value.clone())).collect())?,
        })
    }
    ...
    pub(super) fn apply(&self, model: &mut Project, diagrams: Vec<SharedBddDiagram>) -> Result<Vec<SharedBddDiagram>, CollaborationError> {
        let mut diagram_map: HashMap<_, _> = diagrams.into_iter().map(|diagram| (diagram.id, diagram)).collect();
        ...
        model.validate().map_err(|_| CollaborationError::UndoConflict)?;
        let result: Vec<_> = diagram_map.into_values().collect();
        for diagram in &result {
            super::validate_shared_bdd(model, diagram).map_err(|_| CollaborationError::UndoConflict)?;
        }
        Ok(result)
    }
}
