...
impl SharedBddDiagram {
    ...
    pub fn native_presentation(&self) -> BddDiagram {
        BddDiagram {
            ...
            nodes: self.nodes.iter().map(|node| DiagramNode {
            ...
            edges: self.edges.iter().map(|edge| DiagramEdge {
    ...
    pub(super) fn apply_native_geometry(
        ...
        apply_bdd_geometry(&mut native, command, BddRoutingScope::AllEdges, None)
            .map_err(|_| CollaborationError::InvalidDiagram(
                "native BDD geometry command or obstacle-clear route is invalid",
            ))?;
        let mut candidate = self.clone();
        for node in &mut candidate.nodes {
            let changed = native.nodes.iter().find(|value| value.id == node.id.to_string())
                .ok_or(CollaborationError::InvalidDiagram("native BDD node was not found"))?;
            node.x = changed.x;
            node.y = changed.y;
        ...
        for edge in &mut candidate.edges {
            let changed = native.edges.iter().find(|value| value.id == edge.id.to_string())
                .ok_or(CollaborationError::InvalidDiagram("native BDD edge was not found"))?;
            edge.points = changed.points.clone();
            edge.label_anchor = changed.label_anchor
                .ok_or(CollaborationError::InvalidDiagram("native BDD label was not found"))?;
        }
        *self = candidate;
        Ok(())
    }
