...
use systems_modeler_persistence::collaboration::{CollaborationError, EditRequest, ProjectRole, SharedEdit};
use uuid::Uuid;
...
fn shared_geometry_matches_native_preserves_wire_shape_retries_and_reverses_after_reopen() {
    ...
    let relationship = project.create_relationship(RelationshipKind::Generalization, source, target, Some(project.root_id)).unwrap();
    database.save_project(&project).unwrap();
    let actor = Uuid::new_v4();
    let viewer = Uuid::new_v4();
    database.provision_shared_member(project.id, actor, ProjectRole::Editor).unwrap();
    ...
    for (revision, edit) in [
        ...
        SharedEdit::PresentBddRelationship { diagram, edge, relationship },
    ].into_iter().enumerate() {
        database.commit_shared_edit(project.id, actor, &request(revision as i64, edit)).unwrap();
    }
    ...
    let command = BddGeometryCommand::UpdateNode {
        presentation_id: source_node.to_string(), x: 100.0, y: 360.0, width: 210.0, height: 120.0,
    };
    apply_bdd_geometry(&mut native, &command, BddRoutingScope::AllEdges, None).unwrap();
    let edit = request(revision, SharedEdit::UpdateBddNodeGeometry {
        diagram, node: source_node, x: 100.0, y: 360.0, width: 210.0, height: 120.0,
    });
    assert!(matches!(database.commit_shared_edit(project.id, viewer, &edit), Err(CollaborationError::Forbidden)));
    let receipt = database.commit_shared_edit(project.id, actor, &edit).unwrap();
    let (_, after, after_revision) = database.shared_snapshot(project.id, actor).unwrap();
    assert_eq!(after_revision, revision + 1);
    assert_eq!(serde_json::to_value(after[0].native_presentation()).unwrap(), serde_json::to_value(&native).unwrap());
    let original_keys: Vec<_> = serde_json::to_value(original).unwrap().as_object().unwrap().keys().cloned().collect();
    let after_keys: Vec<_> = serde_json::to_value(&after[0]).unwrap().as_object().unwrap().keys().cloned().collect();
    assert_eq!(original_keys, after_keys);
    assert!(matches!(database.commit_shared_edit(project.id, actor, &request(revision, SharedEdit::RouteBddDiagram { diagram })), Err(CollaborationError::Conflict { .. })));
    let invalid = request(after_revision, SharedEdit::UpdateBddNodeGeometry {
        diagram, node: source_node, x: 100.0, y: 100.0, width: 1.0, height: 1.0,
    });
    assert!(database.commit_shared_edit(project.id, actor, &invalid).is_err());
    ...
    assert_eq!(database.shared_history(project.id, actor).unwrap().len(), after_revision as usize);
    drop(database);
    let database = ProjectDatabase::open(&path).unwrap();
    assert_eq!(database.commit_shared_edit(project.id, actor, &edit).unwrap(), receipt);
    database.commit_shared_edit(project.id, actor, &request(after_revision, SharedEdit::UndoOperation { operation: edit.operation_id })).unwrap();
