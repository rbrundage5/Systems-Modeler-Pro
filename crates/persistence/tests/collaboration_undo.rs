...
use systems_modeler_persistence::{
    ProjectDatabase,
    collaboration::{CollaborationError, EditRequest, ProjectRole, SharedEdit},
};
use uuid::Uuid;
...
fn member(database: &mut ProjectDatabase, project: ProjectId) -> Uuid {
    ...
    database
        .provision_shared_member(project, actor, ProjectRole::Editor)
        .unwrap();
    actor
}
...
fn reversal_preserves_other_users_work_and_retries_after_reopen() {
    ...
    let first = request(
        ...
        SharedEdit::CreateBlock {
            ...
            name: "Alice block".into(),
        },
    );
    let alice_block = database
        .commit_shared_edit(project.id, alice, &first)
        .unwrap()
        .element;
    let bob_block = database
        .commit_shared_edit(
            project.id,
            bob,
            &request(
    ...
    assert_eq!(
        database
            .commit_shared_edit(project.id, alice, &reverse)
            .unwrap()
            .revision,
        3
    );
    ...
    database
        .commit_shared_edit(project.id, alice, &undo(3, reverse.operation_id))
        .unwrap();
    let (model, _, revision) = database.shared_snapshot(project.id, alice).unwrap();
    ...
    assert!(matches!(
        database.commit_shared_edit(project.id, alice, &undo(4, first.operation_id)),
        Err(CollaborationError::UndoUnavailable)
    ));
    ...
    rusqlite::Connection::open(&path)
        .unwrap()
        .execute("DELETE FROM shared_undo", [])
        .unwrap();
    assert!(
        database
            .shared_history(project.id, alice)
            .unwrap()
            .iter()
...
fn changed_objects_other_authors_and_viewers_cannot_be_overwritten() {
    ...
    let block = project
        .create_element(ElementKind::Block, "Original", project.root_id)
        .unwrap();
    database.save_project(&project).unwrap();
    let alice = member(&mut database, project.id);
    let bob = member(&mut database, project.id);
    ...
    database
        .commit_shared_edit(project.id, alice, &change)
        .unwrap();
    database
        .commit_shared_edit(
            project.id,
    ...
    let before = snapshot(&database, project.id, alice);
    assert!(matches!(
        database.commit_shared_edit(project.id, alice, &undo(2, change.operation_id)),
        Err(CollaborationError::UndoConflict)
    ));
    assert!(matches!(
        database.commit_shared_edit(project.id, bob, &undo(2, change.operation_id)),
        Err(CollaborationError::Forbidden)
    ));
    assert!(matches!(
        database.commit_shared_edit(project.id, viewer, &undo(2, change.operation_id)),
        Err(CollaborationError::Forbidden)
    ));
    assert_eq!(snapshot(&database, project.id, alice), before);
    ...
    assert!(
        database
            .shared_history(project.id, viewer)
            .unwrap()
            .is_empty()
    );
}
...
fn new_dependencies_block_reversal_until_the_author_removes_them() {
    ...
    let owner = database
        .commit_shared_edit(project.id, alice, &package)
        .unwrap()
        .element;
    let child = request(
        1,
    ...
    database
        .commit_shared_edit(project.id, bob, &child)
        .unwrap();
    let before = snapshot(&database, project.id, alice);
    assert!(matches!(
        database.commit_shared_edit(project.id, alice, &undo(2, package.operation_id)),
        Err(CollaborationError::UndoConflict)
    ));
    assert_eq!(snapshot(&database, project.id, alice), before);
    database
        .commit_shared_edit(project.id, bob, &undo(2, child.operation_id))
        .unwrap();
    database
        .commit_shared_edit(project.id, alice, &undo(3, package.operation_id))
        .unwrap();
    assert_eq!(
        database
            .shared_snapshot(project.id, alice)
            .unwrap()
            .0
            .elements
            .len(),
...
fn diagram_node_and_edge_restore_together_without_removing_another_diagram() {
    ...
    for (revision, edit) in edits.into_iter().enumerate() {
        database
            .commit_shared_edit(project.id, alice, &request(revision as i64, edit))
            .unwrap();
    }
    ...
    database
        .commit_shared_edit(project.id, alice, &removal)
        .unwrap();
        ...
        .commit_shared_edit(
            ...
            &request(
                ...
                SharedEdit::CreateBddDiagram {
                    diagram: other,
                    owner: project.root_id,
                    name: "Bob diagram".into(),
                },
            ),
        )
        .unwrap();
    database
        .commit_shared_edit(project.id, alice, &undo(6, removal.operation_id))
        .unwrap();
    let (model, diagrams, revision) = database.shared_snapshot(project.id, alice).unwrap();
    assert_eq!(revision, 7);
    assert_eq!(
        diagrams.iter().find(|entry| entry.id == diagram),
        Some(&original)
    );
