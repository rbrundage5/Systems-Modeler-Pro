...
fn requirements_and_traceability_round_trip_without_identity_changes() {
    ...
    assert!(
        restored
            .children(package)
            .any(|element| element.id == requirement)
    );
    assert_eq!(
        restored.relationship(relationship).unwrap().owner_id,
        Some(package)
    );
    assert_eq!(
        restored.relationship(relationship).unwrap().source_id,
        block
    );
    assert_eq!(
        restored.relationship(relationship).unwrap().target_id,
        requirement
    );
