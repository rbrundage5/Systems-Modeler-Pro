...
fn pending_operation_survives_reopen_and_is_scoped_to_actor_and_server() {
    ...
    assert_eq!(
        recovered.load(server, actor).unwrap(),
        Some(original.clone())
    );
    assert!(recovered.load(server, Uuid::new_v4()).unwrap().is_none());
    assert!(
        recovered
            .load("https://other.example/", actor)
            .unwrap()
            .is_none()
    );
...
fn another_application_cannot_overwrite_or_clear_a_different_operation() {
    ...
    first
        .reserve("https://models.example/", actor, &original)
        .unwrap();
    assert!(
        second
            .reserve("https://models.example/", actor, &other)
            .is_err()
    );
    assert!(
        second
            .complete("https://models.example/", actor, &other)
            .is_err()
    );
    assert_eq!(
        first.load("https://models.example/", actor).unwrap(),
        Some(original.clone())
    );
    second
        .reserve("https://models.example/", actor, &original)
        .unwrap();
    first
        .complete("https://models.example/", actor, &original)
        .unwrap();
    second
        .complete("https://models.example/", actor, &original)
        .unwrap();
    ...
    assert_eq!(
        second.load("https://models.example/", actor).unwrap(),
        Some(other)
    );
}
...
fn malformed_record_is_preserved_and_reported_instead_of_replaced() {
    ...
    let mut outbox = CollaborationOutbox::open(&path).unwrap();
    outbox
        .reserve("https://models.example/", actor, &request)
        .unwrap();
    let connection = rusqlite::Connection::open(&path).unwrap();
    connection
        .execute(
            "UPDATE pending_shared_edits SET request_json='invalid json'",
            [],
        )
        .unwrap();
    ...
    let saved: String = connection
        .query_row("SELECT request_json FROM pending_shared_edits", [], |row| {
            row.get(0)
        })
        .unwrap();
