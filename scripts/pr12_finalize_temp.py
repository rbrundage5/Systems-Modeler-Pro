...
ws = "apps/desktop/src-tauri/src/workspace/behavior_workspace.rs"
replace_once(ws, '''#[derive(Debug, Clone, Serialize, Deserialize)]
...
replace_once(ws, '''    diagram.lifelines.push(LifelinePresentation {
...
replace_once(ws, '''#[tauri::command]
#[allow(clippy::too_many_arguments)] // Stable named-field Tauri IPC boundary.
pub fn add_sequence_message(''', '''#[tauri::command]
...
with Path(ws).open("a", encoding="utf-8") as f:
    f.write('''

#[cfg(test)]
mod lifeline_presentation_tests {
    use super::{
        LifelinePresentation, default_lifeline_timeline_end_y,
        default_lifeline_timeline_start_y,
    };

    #[test]
    fn legacy_lifeline_presentation_receives_timeline_defaults() {
        let presentation: LifelinePresentation = serde_json::from_str(
            r#"{"lifeline_id":"legacy","x":240.0}"#,
        )
        .expect("legacy Lifeline presentation should deserialize");
        assert_eq!(presentation.timeline_start_y, default_lifeline_timeline_start_y());
        assert_eq!(presentation.timeline_end_y, default_lifeline_timeline_end_y());
        assert!(presentation.timeline_end_y - presentation.timeline_start_y >= 80.0);
    }
}
''')

main = "apps/desktop/src-tauri/src/main.rs"
replace_once(main, '''        create_state_machine_diagram, move_sequence_lifeline, move_state_vertex,
        update_state_behaviors,''', '''        create_state_machine_diagram, move_sequence_lifeline, move_state_vertex,
        resize_sequence_lifeline_timeline, update_state_behaviors,''')
replace_once(main, '''    ibd_item_flow_notation, move_sequence_lifeline, move_state_vertex, new_project,''', '''    ibd_item_flow_notation, move_sequence_lifeline, move_state_vertex,
    resize_sequence_lifeline_timeline, new_project,''')
replace_once(main, '''            move_sequence_lifeline,
            add_sequence_message,''', '''            move_sequence_lifeline,
            resize_sequence_lifeline_timeline,
            add_sequence_message,''')

renderer = "apps/desktop/frontend/behavior-authoritative-renderer.js"
replace_once(renderer, '''  function lifelinePresentation(diagram, id, index) {
...
replace_once(renderer, '''      node.style.left = `${presentation.x - 65}px`;''', '''      const timelineStart = Number.isFinite(presentation.timeline_start_y) ? presentation.timeline_start_y : 102;
...
replace_once(renderer, '''      node.innerHTML = `<div class="lifeline-head">${escapeHtml(lifeline.name || 'Lifeline')}</div><div class="lifeline-line"></div>`;''', '''      node.innerHTML = `<div class="lifeline-head">${escapeHtml(lifeline.name || 'Lifeline')}</div><div class="lifeline-line"></div><span class="lifeline-resize-handle" title="Resize Lifeline timeline" aria-label="Resize Lifeline timeline"></span>`;
...
replace_once(renderer, '''      if (!presentation.fallback) {
        node.onpointerdown = (event) => {
          if (state.behaviorPending || state.behaviorTool) return;''', '''      if (!presentation.fallback) {
...
css = Path("apps/desktop/frontend/behavior.css")
css.write_text(css.read_text(encoding="utf-8") + '.lifeline-resize-handle{position:absolute;left:56px;width:17px;height:11px;border:1px solid #555;border-radius:2px;background:#fff;cursor:ns-resize;z-index:8;box-sizing:border-box}.sequence-lifeline.selected .lifeline-resize-handle{border-width:2px}.execution-spec.selected,.combined-fragment.selected,.state-invariant-box.selected{outline:3px solid #4a79b8;outline-offset:2px}\n', encoding="utf-8")

contract = "scripts/validate_behavior_integration.py"
replace_once(contract, '''    "move_sequence_lifeline",
    "add_sequence_message",''', '''    "move_sequence_lifeline",
    "resize_sequence_lifeline_timeline",
    "add_sequence_message",''')
replace_once(contract, '''    "move_sequence_lifeline",
    "Reply",''', '''    "move_sequence_lifeline",
    "resize_sequence_lifeline_timeline",
...
'''
if anchor not in s:
    raise SystemExit("contract final anchor missing")
