...
assert "y.clamp(rect.y + margin, rect.y + rect.height - margin)" in ibd_geometry_rs, "IBD ports are not boundary constrained"
assert "preview_ibd_port_geometry" in frontend and "config.constrain" in frontend
assert "history::edit_ibd_geometry" in interaction_rs and "pub context_frame:" in ibd_rs
assert "frame_port_history_and_native_round_trip_are_atomic" in ibd_geometry_rs
