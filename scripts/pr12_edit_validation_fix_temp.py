...
validation = r'''use std::collections::{HashMap, HashSet};
...
fn validate_trigger(project: &Project, trigger: Option<&Trigger>) -> Result<(), String> {
    ...
    match &trigger.event {
        Event::Signal { signal_id } => {
            if project.element(*signal_id).map_err(|error| error.to_string())?.kind
                != ElementKind::Signal
            {
                return Err(format!("Signal trigger must reference a Signal: {signal_id}"));
            }
        }
...
mod tests {
    ...
    fn editing_rejects_impossible_initial_final_fork_join_topology() {
        ...
        assert!(validate_state_machine_editing(&project, machine)
            .unwrap_err()
            .contains("only one outgoing"));
    }
}
'''
Path("apps/desktop/src-tauri/src/workspace/behavior_completion/validation.rs").write_text(validation, encoding="utf-8")

replace_once(
    "apps/desktop/src-tauri/src/workspace/behavior_completion/mod.rs",
    "mod transition;\n",
    "mod transition;\nmod validation;\n",
)

replace_once(
    "apps/desktop/src-tauri/src/workspace/behavior_completion/transition.rs",
...
replace_once(
    "apps/desktop/src-tauri/src/workspace/behavior_completion/delete.rs",
    "use super::super::behavior_workspace::BehaviorDiagramKind;\n",
    "use super::super::behavior_workspace::BehaviorDiagramKind;\nuse super::validation::validate_state_machine_editing;\n",
)
replace_once(
...
replace_once(
    "apps/desktop/src-tauri/src/workspace/behavior_completion/state.rs",
...
replace_once(
    ...
    '    "add_state_transition_complete",\n    "validate_state_machine_editing",\n',
)
p = Path(contract)
text = p.read_text(encoding="utf-8")
...
if anchor not in text:
    raise SystemExit("integration contract final anchor missing")
p.write_text(text.replace(anchor, extra + anchor, 1), encoding="utf-8")
