...
# Compile the Rust behavior semantic engine as part of the authoritative model core.
replace(
...
# Persist behavior semantics and diagram presentation with the project.
...
old = """    ibd::save_ibd_metadata(&mut database, project, &ibd_diagrams)?;
    let saved_path = path.to_string_lossy().into_owned();"""
new = """    ibd::save_ibd_metadata(&mut database, project, &ibd_diagrams)?;
    ...
    let saved_path = path.to_string_lossy().into_owned();"""
if old not in s:
    raise SystemExit("save behavior anchor missing")
s = s.replace(old, new, 1)
old = """    let ibd_diagrams = ibd::load_ibd_metadata(&database, &project)?;
    let opened_path = path.to_string_lossy().into_owned();"""
new = """    let ibd_diagrams = ibd::load_ibd_metadata(&database, &project)?;
    let (behavior, behavior_diagrams) = behavior_workspace::load_behavior_metadata(&database, &project)?;
    let opened_path = path.to_string_lossy().into_owned();"""
if old not in s:
    raise SystemExit("open behavior anchor missing")
s = s.replace(old, new, 1)
old = """    *state.ibd_diagrams.lock().map_err(|_| \"IBD lock poisoned\")? = ibd_diagrams;
    *state.current_file.lock().map_err(|_| \"project path lock poisoned\")? = Some(opened_path.clone());"""
new = """    *state.ibd_diagrams.lock().map_err(|_| \"IBD lock poisoned\")? = ibd_diagrams;
    ...
    *state.current_file.lock().map_err(|_| \"project path lock poisoned\")? = Some(opened_path.clone());"""
if old not in s:
    raise SystemExit("assign behavior anchor missing")
s = s.replace(old, new, 1)
p.write_text(s)

# Register behavior workspace/Tauri commands.
...
addition = anchor + """
    pub use behavior_workspace::{
        ...
        add_state_vertex, behavior_lifeline_candidates, behavior_snapshot,
        create_sequence_diagram, create_state_machine_diagram, move_sequence_lifeline,
        move_state_vertex, update_state_behaviors,
    };"""
if anchor not in s:
    raise SystemExit("main behavior export anchor missing")
...
s = s.replace(
    ...
    behavior_snapshot, create_bdd,""",
    1,
)
s = s.replace(
    """    create_package, delete_bdd_relationship, ibd_item_flow_notation, new_project,""",
    """    create_package, create_sequence_diagram, create_state_machine_diagram,
...
# Load professional State Machine/Sequence notation and UI.
p = Path("apps/desktop/frontend/index.html")
s = p.read_text()
s = s.replace(
    '  <link rel="stylesheet" href="ibd.css" />',
    '  <link rel="stylesheet" href="ibd.css" />\n  <link rel="stylesheet" href="behavior.css" />',
    1,
)
s = s.replace(
    '<script src="item-flow-ui.js"></script><script src="ui-shell.js"></script>',
    '<script src="item-flow-ui.js"></script><script src="behavior-ui.js"></script><script src="ui-shell.js"></script>',
    1,
)
p.write_text(s)

# Restore permanent CI and remove one-time scaffolding from the product commit.
Path(".github/workflows/ci.yml").write_text("""name: native-foundation-ci
...
jobs:
  core:
    ...
    steps:
      ...
      - name: Core tests
        run: cargo test -p systems-modeler-core -p systems-modeler-persistence
      - name: Core lint
        run: cargo clippy -p systems-modeler-core -p systems-modeler-persistence --all-targets -- -D warnings
  ...
  desktop-check:
    ...
    steps:
      ...
      - name: Verify Cargo.lock remains clean
        shell: pwsh
        run: git diff --exit-code -- Cargo.lock
""")
for name in [
    ".github/workflows/pr11-dedupe-itemflows.yml",
    ".github/workflows/pr12-bootstrap.yml",
    ".github/workflows/pr12-fix-clippy.yml",
    ".github/workflows/pr12-integrate-behavior.yml",
    "scripts/pr12_integrate.py",
]:
    q = Path(name)
    if q.exists():
        q.unlink()
